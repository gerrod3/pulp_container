...
class TestPullThroughDistributionRepair(TestCase):
    def test_command_repairs_eligible_distributions_and_reports_the_rest(self):
        repairable = ContainerPullThroughDistribution.objects.create(
            name="repairable", base_path="repairable"
        )
        unresolved = ContainerPullThroughDistribution.objects.create(
        ...
        self.assertEqual(unresolved.base_path, "different")
        self.assertIn("Repaired 1 pull-through distributions.", output.getvalue())
        self.assertIn("name='unresolved'", output.getvalue())
...
class TestPullThroughDistributionLookup(TestCase):
    def test_marked_distribution_matches_on_name(self):
        distribution = ContainerPullThroughDistribution.objects.create(
            name="registry-cache",
            base_path=str(uuid4()),
            pulp_labels=PULL_THROUGH_DISTRIBUTION_MARKER,
        )
